...
bool PIPSocket::SetQoS(const QoS & qos)
{
  ...
  if (PWinSock::GetInstance().m_useGQOS && new_tos < 0 && qos.m_type != BestEffortQoS) 
  {
    ...
    if (ConvertOSError(nTest)) 
	{
      ...
      PTRACE(2, "DEBUG TOS: SetQoS => Using GQOS failed. With errorcode: " << ::GetLastError());PTRACE(2, "DEBUG TOS: SetQoS => Using GQOS failed. With errorcode: " << ::GetLastError());
  ...
  if (PWinSock::GetInstance().m_hQoS != NULL && peer.IsValid()){
    ...
    if (m_qosFlowId == 0){
      ...
      if (!peer.IsValid() && !GetPeerAddress(peer)) {
        ...
        PTRACE_IF(1, !ok, "WinSock", "DEBUG TOS: SetQoS => Could not add socket to QoS flow (2). With errorcode: " << ::GetLastError());
    }
    ...
    if (ok && new_tos >= 0){
      ...
      if (!QOSSetFlow(PWinSock::GetInstance().m_hQoS, m_qosFlowId, QOSSetOutgoingDSCPValue, sizeof(dscp), &dscp, 0, NULL))
        ...
        PTRACE(2, "WinSock","DEBUG TOS: SetQoS => DSCP value set to: " << dscp);
#else
      ok = false;
#endif // P_QWAVE_DSCP
      if (ok){
